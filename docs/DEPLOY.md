...
Telegram-бот, PostgreSQL, Redis. Интерфейс, API и админка отдаются одним nginx
на одном origin — поэтому CORS в этой схеме не участвует.
...
запускать `migrate` не нужно. Фронтенд собирается внутри образа nginx
(`npm run build`), поэтому первая сборка идёт долго.

> **Если на этой же машине запускался dev-стек** (`docker-compose.yml`), задайте
> прод-стеку своё имя проекта: `docker compose -p hero_prod -f ...`. Иначе оба
> файла используют имя каталога и делят одни и те же тома с базой — прод
> подключится к dev-базе с другим паролем и не стартует.

> Контейнер `telegram-bot` будет перезапускаться по кругу, пока не задан
> `TELEGRAM_BOT_TOKEN`: команда намеренно падает с понятной ошибкой,
...
После запуска на одном порту доступно всё:
...
| `/health/`, `/ready/` | состояние сервисов |
| `/metrics/` | закрыт снаружи, только из приватных подсетей |
